function enviwrite(filename,envi_data,datatype,resolu,UL,interleave,zone,vert)
...
if (zone > 0)
    fprintf(fid_out, 'map info = {Transverse Mercator, 1.000, 1.000, %d, %d, %d, %d, North, WGS-84}\n',UL(1),UL(2),resolu(1),resolu(2));
else
    fprintf(fid_out, 'map info = {Transverse Mercator, 1.000, 1.000, %d, %d, %d, %d, North, WGS-84}\n',UL(1),UL(2),resolu(1),resolu(2));
end
 fprintf(fid_out, 'projection info = {3, 6378137, 6356752.314245179, 0, %d, %d, 0, 1,WGS-84, Transverse Mercator}',vert(1),vert(2));
fclose(fid_out);
end
