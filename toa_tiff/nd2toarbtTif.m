function [TOAref,geo,info]=nd2toarbtTif(filepath)
...
% 11) resolution of Fmask results
 ...
 fprintf('Read in header information & TIF images\n');
  file = strcat(filepath,'\*_MTL.txt');
    dd = dir(file);
    filename=strcat(filepath,'\',dd(1).name);
...
% %     end
    ...
    im_B7=single(imread(strcat(filepath,'\',n_B7.name)));
      %read mapinfo
   [image, geo]=geotiffread(strcat(filepath,'\',n_B7.name));
...
elseif Lnum == 8
    ...
    im_B5=single(imread(strcat(filepath,'\',n_B5.name)));
    % Band6
    n_B6=dir(strcat(filepath,'\*B6.TIF'));
    im_B6=single(imread(strcat(filepath,'\',n_B6.name)));
    % Band7
    n_B7=dir(strcat(filepath,'\*B7.TIF'));
    im_B7=single(imread(strcat(filepath,'\',n_B7.name)));
       %read mapinfo
...
%         
%         im_B11=pixel2pixv([ul(2),ul(1)],[ul(2),ul(1)],...
%             resolu,[reso_thm,reso_thm],...
%             im_B11,[ijdim_ref(2),ijdim_ref(1)],[ijdim_thm(2),ijdim_thm(1)]);
%     end
    
    % only processing pixesl where all bands have values (id_mssing)
    id_missing=im_B2==0|im_B3==0|im_B4==0|im_B5==0|im_B6==0|im_B7==0;
    % find pixels that are saturated in the visible bands
%     B1Satu=im_B2==65535;
%     B2Satu=im_B3==65535;
%     B3Satu=im_B4==65535;
...
%     im_B11=((Lmax(9)-Lmin(9))/(Qcalmax(9)-Qcalmin(9)))*(im_B11-Qcalmin(9))+Lmin(9);
...
%     im_B11=K2_B11./log((K1_B11./im_B11)+1);
%     % convert from Kelvin to Celcius with 0.01 scale_facor
%     im_B10=100*(im_B10-273.15);
%     im_B11=100*(im_B11-273.15);
%     % get data ready for Fmask
    TOAref=zeros(ijdim_ref(1),ijdim_ref(2),6,'single');% Band 2,3,4,5,6,7,& 9
    im_B2(id_missing)=-9999;
    im_B3(id_missing)=-9999;
    im_B4(id_missing)=-9999;
    im_B5(id_missing)=-9999;
...
%     im_th=zeros(ijdim_ref(1),ijdim_ref(2),2,'single');% Band 10 & 11
%     im_B10(id_missing) = -9999;
%     im_B11(id_missing) = -9999;
%     im_th(:,:,1) = im_B10;
%     im_th(:,:,2) = im_B11;  
    

else
    fprintf('This sensor is not Landsat 5,or7 8!\n');
    return;
end
end
